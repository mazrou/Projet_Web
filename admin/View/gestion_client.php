  ...
  <div class="collapse navbar-collapse" id="navbarText">
    <ul class="navbar-nav mr-auto p-1">
      ...
      <li class="nav-item active mr-3">
                <a class="nav-link" href="#"> Gestion des clients <span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item mr-3">
...
<br><br><br>
          ...
          <form style="display:block;margin-top:0em;margin-block-end:1em"action="?action=filtrerClient" class="shadow" id="ddt" method="POST">
            <center>
            <button style="width:20%;margin-left:15px;margin-right:15px"type="submit">Filtrer selon</button>
             <SELECT name="filtrer" size="1" required>
                  <OPTION value="1">Clients bloques 
                  <OPTION value="2">Clients non bloques
                  <OPTION value="3">Clients effectuez des demandes
                  <OPTION value="4">Clients n'effectuez pas des demandes
                </SELECT>
             </center>
           </form>
           <br><br>
        ...
        <div class="col-sm-12">
          ...
          <thead>
            <tr>
                ...
                <td style="cursor: pointer;"onclick="sortTable(6)">Tel</td>
                <td style="cursor: pointer;"onclick="sortTable(7)">Fax</td>
                <td style="cursor: pointer;"onclick="sortTable(8)">Nombre de demandes</td>
            ...
            <tbody>
                ...
                ?>
                <tr>
                    <td><a href="?action=afficherDemandeClient&id=<?=$r['id'] ?>"><?php echo htmlspecialchars($r['nom']) ?></a></td>
                    ...
                    <td><?php echo htmlspecialchars($r['fax'])?></td>
                    <td><?php echo htmlspecialchars($r['nb_demande'])?></td>
                    <td><?php echo htmlspecialchars($r['bloquer'])?></td>
                    <td><?php echo htmlspecialchars($r['suppr'])?></td>
                    <td><button><a href="?action=supprimerClient&id=<?=$r['id'] ?>">Supprimer</a></button></td>
                    <td><button><a href="?action=bloquerClient&id=<?=$r['id'] ?>">Bloquer</a></button></td>
                    <td><button><a href="?action=debloquerClient&id=<?=$r['id'] ?>">Debloquer</a></button></td>
